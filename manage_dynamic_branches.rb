...
class BranchJobManager
  ...
  def calc_missing_jobs(git_repo_url, project_name)
    ...
    ds2 = ds.map{|d| d.sub(project_name + "_", '')}.select{ |x| !x.end_with?("template") && !x.end_with?("master") } #shortest form
    to_rem = ds2 - bs2
    to_add = bs2 - ds2
  ...
  def create_job_from_branch(project_name, simple_branch)
    ...
    puts "will create #{job_name} from #{project_name}_template, using branch: #{simple_branch}"
